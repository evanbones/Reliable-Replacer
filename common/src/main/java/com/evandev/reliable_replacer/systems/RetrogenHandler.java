...
public class RetrogenHandler {
    ...
    public static void processChunk(LevelChunk chunk) {
        ...
        if (!ModConfig.get().enabled || !ModConfig.get().enableRetrogen || !RuleManager.HAS_RETROGEN_RULES || (access.reliableReplacer$hasBeenProcessed() && access.reliableReplacer$getRulesHash() == RuleManager.RETROGEN_RULES_HASH && !access.reliableReplacer$isDirty())) {
            return;
        }
